...
import os, time, random, re, json, threading, logging, unicodedata
...
ROI_MIN       = int(os.environ.get('ROI_MIN', 10))
PROFIT_MIN    = int(os.environ.get('PROFIT_MIN', 5))
...
MAX_AGE_H     = int(os.environ.get('MAX_AGE_H', 6))   # ✅ FIX: variable définie
...
def make_slug(text):
    """Génère un slug LBC depuis un titre : minuscules, sans accents, espaces → tirets."""
...
def make_article(titre, prix, frais_port, lien, plateforme, cat, poids_g, titre_fin,
                 img="", ville="", age="récemment"):
    global next_id
    if not lien or lien in seen_urls:
        return None
    ...
    if roi_pct < ROI_MIN or profit < PROFIT_MIN:
        return None
    seen_urls.add(lien)
    next_id += 1
    return {
        ...
        "titre":      titre[:80],
        "prix":       prix,
        "fraisPort":  round(frais_port, 2),
        "valReelle":  round(val, 2),
        "revente":    round(rev, 2),
        "roi":        roi_pct,
        "profit":     profit,
        "lien":       lien,
...
def get_cached_token():
    ...
    if time.time() - _ebay_token_time > 6000:
...
def scrape_lbc(keywords, cat, val_ref, poids_g, titre_fin):
    ...
    try:
        ...
        for ad in resp.json().get("ads", []):
            ...
            # ✅ FIX: filtre âge strict — ignorer les annonces > MAX_AGE_H
            date_str = ad.get("first_publication_date", "")
            try:
                dt   = datetime.fromisoformat(date_str.replace('Z', '+00:00'))
                diff = int((datetime.now(dt.tzinfo) - dt).total_seconds())
                if diff > MAX_AGE_H * 3600:
                    continue  # Trop vieille → ignorer
                age = ("À l'instant" if diff < 60
                       else f"il y a {diff//60}min" if diff < 3600
                       else f"il y a {diff//3600}h")
            except:
                continue  # Pas de date → ignorer par sécurité

            # ✅ FIX: slug généré depuis le titre (minuscules, sans accents, espaces → tirets)
            slug = make_slug(titre_ad)
            lien = f"https://www.leboncoin.fr/ad/{slug}/{list_id}"
            ...
            a = make_article(titre_ad, prix, frais_port, lien, "lbc", cat,
                             poids_g, titre_fin, img, ville, age)
            if a:
                results.append(a)
                stats["lbc_ok"] += 1
...
def scrape_vinted(keywords, cat, val_ref):
    ...
    try:
        ...
        url  = (f"https://www.vinted.fr/api/v2/catalog/items"
                f"?search_text={quote_plus(keywords)}"
                f"&order=newest_first&price_to={prix_max}&per_page=15")
        resp = requests.get(url, headers={
            "User-Agent":      "Mozilla/5.0 (iPhone; CPU iPhone OS 17_4) AppleWebKit/605.1.15 Mobile Safari/604.1",
            "Accept-Language": "fr-FR"
        }, timeout=10)
        if resp.status_code != 200: return results
...
if __name__ == "__main__":
    log.info(f"VAULTFLIP | Port:{PORT} | ROI_MIN:{ROI_MIN}% | MAX_AGE_H:{MAX_AGE_H}h | eBay API:{'OUI' if EBAY_APP_ID else 'NON (HTML fallback)'}")
